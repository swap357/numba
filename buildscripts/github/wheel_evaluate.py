...
WHEEL_BUILD_MATRIX = {
    "linux-64": [
        {"python-version": "3.10",
         "numpy_build": "2.0.2", "python_tag": "cp310"},
        {"python-version": "3.11",
         "numpy_build": "2.0.2", "python_tag": "cp311"},
        {"python-version": "3.12",
         "numpy_build": "2.0.2", "python_tag": "cp312"},
        {"python-version": "3.13",
         "numpy_build": "2.1.3", "python_tag": "cp313"},
        {"python-version": "3.14",
         "numpy_build": "2.3.3", "python_tag": "cp314"},
...
def add_canonical_version(matrix):
    """Add python_canonical field (e.g., '3.14t' -> '3.14')."""
    result = []
    for item in matrix:
        item_copy = item.copy()
        py_version = item["python-version"]
        # Strip free-threaded suffix before splitting
        canonical = py_version.rstrip("t")
...
def evaluate(event, label, platform, inputs="{}"):
    ...
    base_build = WHEEL_BUILD_MATRIX.get(platform, [])
    base_test = WHEEL_TEST_MATRIX.get(platform, [])
    ...
    build_matrix = add_canonical_version(build_matrix)
    test_matrix = add_canonical_version(test_matrix)
